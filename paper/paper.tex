...
\title{\bf Measuring, Not Classifying:\\ Physically Falsifiable 2D-Material Thickness Estimation\\ from a Single Micrograph}
...
\begin{abstract}
Automated thickness estimation for exfoliated two-dimensional flakes is posed
almost everywhere as classification into discrete layer bins, with thin-film optics
used offline to manufacture training data. A classifier's output cannot be checked
against anything but its labels. We instead place a differentiable transfer-matrix
model of the full optics-to-color pipeline \emph{inside} the inference loop and
recover thickness as a continuous physical quantity by profile-likelihood
continuation, jointly with the imaging nuisances (oxide thickness, sensor gains,
illuminant spectrum). The bare substrate present in every micrograph acts as an
in-frame calibration target, replacing the field's domain-adaptation problem with a
self-calibration problem the image itself solves, and requiring no training data;
supporting a new material is one dispersion table. The consequence we develop here
is that the estimator's outputs must agree with each other and with independently
known constants, which makes them falsifiable in a way a class label is not.

The measurements. On held-out simulated scenes with adversarially misspecified
illuminants, median absolute error is 0.104\,nm for graphene and 0.154\,nm for
MoS$_2$ zero-shot, and 0.0146\,nm at monolayer graphene, a few percent of one atomic
layer. On real micrographs from an external laboratory, layer agreement is 97.0\%
at $n{=}200$ and 95.3\% at $n{=}600$ with no labeled training data, and the inferred
thickness rises by $0.3380 \pm 0.0043$\,nm per annotated layer against graphene's
independently known $0.335$\,nm spacing --- a constant supplied to the estimator at
no point. Repartitioned on the exfoliation run, 21 acquisitions recover their own
substrate oxide thickness from the images with no annotations, spanning
$89.2$--$101.6$\,nm.

Falsifiability then pays twice more. Per-flake inversion is ridge-limited:
nuisances drift jointly with thickness until a wrong interference order fits the
frame, and on real micrographs a median 25\% of flakes per acquisition settle there,
undetected by the usual confidence signal because the wrong solutions are unimodal.
But a displaced thickness necessarily arrives with a displaced oxide, and oxide must
be common to every flake on one wafer. Rejecting flakes whose fitted oxide departs
from their acquisition's median by more than $10$\,nm --- no labels, no wafer
specification, no extra acquisition --- leaves 408 of 665 estimates across 19
acquisitions with \emph{no} failures among them, catching all 194; counting
posterior modes catches 2.2\% of the same failures. And pointed at the field's
public benchmark, the same consistency requirements surfaced three properties its
documentation does not state, including two nominally independent datasets that
share 162 byte-identical images, and imagery that is radiometrically linear where
assuming otherwise rescales thickness twofold with a clean residual. The same
machinery falsified one of our own proposed remedies, which we withdraw. Code and
experiments are released.
\end{abstract}
...
\begin{table}[t]
\centering\small
\caption{Per-exfoliation-run recovery on the MaskTerial graphene corpus, full joint mode (illuminant free, per-flake nuisances). Acquisitions are recovered from \texttt{meta\_data/\{split\}\_set\_name\_to\_uuid.json} and deduplicated across the \texttt{L}/\texttt{M}/\texttt{H} releases. The fitted oxide uses \emph{no} annotations: flakes are selected only by the estimator's own reported uncertainty ($\mathrm{sd}<0.05$\,nm) and summarized by median and MAD, which is robust to the ridge failures of \S\ref{sec:ridges}. Failure rate is the fraction of flakes more than 1\,nm from label\,$\times$\,layer height; it uses labels and is reported as an outcome, not applied as a filter. Exposure is the fitted green gain and $B/G$ the chromatic ratio.}
\label{tab:peracq}
\begin{tabular}{lrrrrr}
\toprule
Exfoliation run & $n$ & oxide (nm) & MAD & fail & $B/G$ \\
\midrule
\multicolumn{6}{l}{\emph{Main group}} \\
Jan3-22-04-2022 & 39 & 89.20 & 3.97 & 38\% & 0.81 \\
Hristiyana\_Cleanroom-22-04-2022 & 40 & 89.52 & 4.45 & 42\% & 0.76 \\
Taoufiq\_Exfol-16-05-2022 & 40 & 90.55 & 4.77 & 45\% & 0.70 \\
Calib-04-04-2022 & 40 & 90.75 & 2.95 & 22\% & 0.71 \\
Jan-22-04-2022 & 22 & 91.63 & 1.85 & 36\% & 0.70 \\
Jan\_Old-23-02-22 & 35 & 91.82 & 4.17 & 37\% & 0.64 \\
P\_2\_Dataset\_ML-07102022 & 13 & 92.15 & 0.93 & 38\% & 0.45 \\
2022-12-06\_Graphite & 40 & 92.60 & 3.80 & 25\% & 0.86 \\
MULTI-04-04-2022 & 29 & 94.33 & 2.34 & 21\% & 0.60 \\
Jan\_LegacyScan70-24-02-2022 & 32 & 94.77 & 4.74 & 19\% & 0.63 \\
2022-07-11\_Graphite & 40 & 95.36 & 1.73 & 8\% & 0.75 \\
tom\_graphen\_13.06.2022\_-2 & 37 & 96.27 & 2.74 & 32\% & 0.72 \\
P\_3\_Dataset\_ML-14102022 & 21 & 96.38 & 4.56 & 14\% & 0.53 \\
Phillip-14-04-2022 & 37 & 99.75 & 3.25 & 24\% & 0.77 \\
Sunaja-20-04-2022 & 40 & 100.90 & 3.99 & 25\% & 0.77 \\
H\_batch\_graphene & 40 & 101.56 & 3.45 & 22\% & 0.56 \\
\midrule
\emph{median, 16 runs, 545 flakes} & & \textbf{93.47} & 4.09 & 25\% & 0.71 \\
\midrule
\multicolumn{6}{l}{\emph{White-balance signature (fitted exposure $>6$)}} \\
Kish\_graphite\_18102023\_V2 & 40 & 107.52 & 6.36 & 40\% & 0.39 \\
Graphene-Graphene\_ID03\_Testset & 40 & 108.35 & 7.52 & 52\% & 0.26 \\
graphene\_271023 & 40 & 108.57 & 6.68 & 38\% & 0.41 \\
GRAPHENE\_20231019 & 40 & 109.34 & 5.45 & 25\% & 0.38 \\
\emph{median, 4 runs} & & \textbf{108.46} & 0.74 & 39\% & 0.38 \\
\midrule
\multicolumn{6}{l}{\emph{Refused by the self-consistency check}} \\
P\_1\_Dataset\_ML-04102022 & 34 & 91.90 & 33.67 & 68\% & 0.56 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Per-exfoliation-run recovery: the full joint estimate on real images.} The
single-oxide assumption above is an artifact of treating a release as a wafer. The
released metadata identifies the exfoliation run behind every image, so we
repartitioned the corpus on that variable, deduplicating images shared between
releases, and ran the \emph{full} joint estimator on each run: illuminant free,
per-flake nuisances, no restricted mode and no labels used to fit the oxide.
Twenty-one runs, 739 flakes. Table~\ref{tab:peracq} reports the result.

Two groups separate on label-free criteria before any accuracy is scored. One
acquisition is refused by the self-consistency check of \S\ref{sec:baselines}: its
fitted oxide scatters with a MAD of $33.7$\,nm, far beyond any wafer tolerance,
which is precisely the falsifiable condition under which we state the numbers are
void. Four further acquisitions separate on fitted exposure, at $6.2$--$7.6$
against the rest of the corpus at $1.7$--$5.0$, with chromatic ratios $B/G$ of
$0.26$--$0.41$ against a median of $0.71$. These are the white-balanced
acquisitions, and they are where the substrate term of \eqref{eq:map} genuinely
does become vacuous: three free gains reproduce a neutral substrate exactly, so
the oxide is left to the flakes and the prior. They fit at $108.46$\,nm with a MAD
of $0.74$ across four independent acquisitions --- tight among themselves and
$15$\,nm above the rest. Whether that is a different wafer or a systematic
displacement caused by the uninformative substrate term cannot be settled from
these images, and we report it as unresolved.

The remaining sixteen acquisitions, 545 flakes, recover substrate oxide from
$89.2$ to $101.6$\,nm with a median of $93.47$\,nm and a between-run MAD of
$4.09$\,nm, against per-run MADs of $0.9$--$4.8$\,nm. That is the wafer
distribution of the corpus, measured from the images with no annotations, and its
span is consistent with the $\sim$5, $\sim$10 and $\sim$20\,nm of oxide spread the
dataset authors report designing into the \texttt{L}, \texttt{M} and \texttt{H}
releases \cite{uslu2024maskterial}. On two runs for which an independent
restricted-mode calibration was also performed, the two routes --- which share no
code path --- agree to $0.6$\,nm (\texttt{Calib} $90.75$ against $93.5$;
\texttt{Jan3} $89.20$ against $90.0$).

The cost is stated plainly. Per-flake thickness inversion in this mode is
ridge-limited: a median of $25\%$ of flakes per acquisition land in a spurious
basin near $35$--$41$\,nm oxide, ranging from $7.5\%$ to $45\%$ across runs. Those
failures are almost all unimodal and the ambiguity flag does not catch them, which
is \S\ref{sec:ridges} holding on real data rather than in simulation. Median
absolute residual on the main group is $0.095$\,nm. Restricted mode remains the
better route for per-flake layer counting; the full joint mode is what recovers
the wafer.

\subsection{Detecting the ridge from the fitted nuisance}
\label{sec:gate}

The protocols of \S\ref{sec:protocols} were designed on the premise that the joint
ridge is an information deficit, remediable only by acquiring more data. Two
experiments on real micrographs change that conclusion.

\textbf{Multi-region does not transfer to real data.} We ran the mode-lattice
protocol on 60 within-image region pairs of differing annotated class across four
exfoliation runs, pairing regions imaged under one illuminant, one exposure and one
wafer. The comparison is paired on identical flakes, since the joint routine also
produces the two single-region estimates. Pooled, it fixed 7 failures and
introduced 7 (exact McNemar $p{=}1.0$). Stratifying by how many regions were
already wrong is the informative view: where both were right, nothing changed
(24 pairs); where both were wrong, nothing was rescued (6 pairs, 0 fixed); and
where exactly one was wrong, it fixed and broke at identical rates (30 pairs, 7
and 7). The shared nuisance vector is as able to drag a correct region onto its
partner's ridge as to exclude it. On one acquisition the joint fit placed both
regions at $35.5$\,nm oxide against an $89.5$\,nm wafer. The synthetic result of
\S\ref{sec:protocols} holds only where every scene is well conditioned by
construction, and we withdraw it as a real-data remedy.

\textbf{The ridge is nonetheless detectable, from a quantity already computed.}
\S\ref{sec:ridges} establishes that the failure is a \emph{joint} drift: nuisances
move continuously with thickness until a wrong interference order fits the frame.
A displaced thickness therefore arrives with a displaced oxide, necessarily. The
three mitigations \S\ref{sec:ridges} reports as failing --- mode counting, fixed
nuisance metamer scanning, discrimination scores --- are all internal to one
flake's posterior, where the wrong solution is genuinely unimodal. The oxide is
different in kind: it is the one nuisance that must be \emph{common to every flake
on a wafer}. Testing it is therefore a cross-flake test, and that is why it
succeeds where within-flake tests cannot.

We reject any flake whose fitted oxide lies more than $10$\,nm from the median
fitted oxide of its own acquisition. No annotations, no wafer specification, no
calibration sweep and no second frame enter this rule; it is the population
consistency check of \S\ref{sec:baselines} applied per flake instead of as a
verdict on a whole run. Across the 19 acquisitions where fewer than half the
flakes fail, 665 flakes: \textbf{408 estimates survive the gate and none of them
is a failure}, while all 194 failures are rejected. Recall is $100\%$ at a
precision of $75.5\%$, so roughly $13\%$ of correct estimates are discarded
alongside them. By comparison the mode-count ambiguity flag fires on $2.0\%$ of
flakes and catches $2.2\%$ of failures.

The rule has a stated domain and a test for it. The median is only a reference
while most flakes are right; on an acquisition where the majority land on the
ridge, the median \emph{is} the ridge and the gate inverts, which is exactly what
the two excluded acquisitions show ($67.6\%$ and $52.5\%$ failure; the gate then
retains 37 flakes of which all 37 are wrong). Both are independently identifiable
without labels --- one is already refused by the population check at a fitted-oxide
MAD of $33.7$\,nm, the other carries the white-balance signature of
Table~\ref{tab:peracq} --- so the two tiers compose: refuse the acquisition first,
then gate the flakes within it.

We report this as a strong empirical regularity on one material, 665 flakes and 19
acquisitions, not as a proof. What would falsify it is a ridge partner that
happens to sit at the wafer's own oxide, which the geometry does not forbid.
...
\appendix
\section{Supplementary: synthetic characterization}
\label{sec:si}
The experiments below establish that the estimator is sound on simulated data and quantify its sensitivities. They are reported in full for completeness; the contribution of this paper is the real-data audit of \S\ref{sec:real}.
